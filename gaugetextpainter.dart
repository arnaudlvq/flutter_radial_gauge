    ...
    for (var i = 0; i <= end; i++) {
      ...
      if(i != 0 && i != end){ //(end / 1.5).toInt() > i && i != 0
        if(secondsMarker == SecondsMarker.all){
          canvas.drawLine(Offset(0.0, -radius - 21), Offset(0.0, -radius - 15 + tickMarkLength), tickPaint);
        }else if(secondsMarker == SecondsMarker.minutes){
          if(i % 5 == 0){
            canvas.drawLine(Offset(0.0, -radius - 10), Offset(0.0, -radius - 15 + tickMarkLength), tickPaint);
          }
        }else if(secondsMarker == SecondsMarker.secondsAndMinute){
          if(i % 5 == 0){
            canvas.drawLine(Offset(0.0, -radius + 20), Offset(0.0, -radius + 12), tickPaint);
          }else{
            canvas.drawLine(Offset(0.0, -radius + 18), Offset(0.0, -radius + 12), tickPaint);
          }
        }else if(secondsMarker == SecondsMarker.seconds){
          canvas.drawLine(Offset(0.0, -radius - widthCircle/2), Offset(0.0, -radius + widthCircle/2 ), tickPaint);
        }
...
class GaugeTextCounter extends CustomPainter {
  ...
  GaugeTextCounter({required this.isDecimal,required this.width,required this.counterAlign,required this.start, required this.end, required this.value,required this.fontFamily,required this.textStyle,})
      : tickPaint = Paint(),
        textPainter = TextPainter(
          textAlign: TextAlign.center,
          textDirection: TextDirection.rtl,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i <= 60; i++) {
      ...
      if (i == 30) {
        ...
        textPainter.paint(canvas, Offset(-(textPainter.width / 2), counterAlign == CounterAlign.center ?-width :0));
